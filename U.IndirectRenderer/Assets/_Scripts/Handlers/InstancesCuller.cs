...
public class InstancesCuller
{
    ...
    public InstancesCuller(ComputeShader computeShader, RendererDataContext context, Camera camera)
    {
        ...
        _camera = camera;
        _threadGroupX = Mathf.Max(1, _context.MeshesCount / 64);
    }

    public void Initialize(List<Vector3> positions, List<Vector3> scales, IndirectRendererSettings settings,
        HierarchicalDepthBufferConfig hiZBufferConfig)
    {
    ...
    public void Dispatch()
    {
        ...
        _computeShader.Dispatch(ShaderKernels.InstancesCuller, _threadGroupX, 1, 1);
    }
